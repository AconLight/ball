...
public class TimeBackElement extends ReversableObject {
	...
	public void render(SpriteBatch batch, int priority) {
		...
		elementSprite.visibility = Math.max(137f/(101+((ReversableMovement)movement).framesI) - 0.4f, 0);
		if (137f/(101+((ReversableMovement)movement).framesI) < 0.35f && parentBack.fadeAway) {
			parentBack.stop();
		}
